...
class OidcJwtHelper
{
   ...
   */
    private static function base64UrlDecode(string $base64url): string|false
    {
   ...
   */
    public function decodeJwt(string $jwt, int $section = 0): object
    {
        ...
        return json_decode(self::base64UrlDecode($parts[$section]));
    }
    ...
    private function getKeyForHeader($keys, $header): mixed
    {
        ...
        if (isset($header->kid)) {
            throw new OidcAuthenticationException(
                sprintf(
                    'Unable to find a key for (algorithm, kid): %s, %s',
                    $header->alg,
                    $header->kid
                )
            );
