...
{
    public const OIDC_SESSION_NONCE = 'oidc.session.nonce';
    public const OIDC_SESSION_STATE = 'oidc.session.state';
  ...
  /** OIDC configuration values */
    ...
    public function __construct(
      ...
      // Check for required phpseclib classes
        if (
            !class_exists('\phpseclib\Crypt\RSA') &&
            !class_exists('\phpseclib3\Crypt\RSA')
        ) {
            throw new RuntimeException(
                'Unable to find phpseclib Crypt/RSA.php.  Ensure phpseclib/phpseclib is installed.'
            );
        }
      ...
      // Check whether the request has an error state
        if ($request->request->has('error')) {
            throw new OidcAuthenticationException(sprintf(
                ...
                $request->request->get('error_description', '')
            ));
        }
    ...
    private function resolveConfiguration(): void
    {
      // Check whether the configuration is already available
        ...
        try {
            ...
            throw new OidcConfigurationResolveException(
                sprintf(
                    'Could not retrieve OIDC configuration from "%s".',
                    $this->wellKnownUrl
                ),
                0,
                $e
      ...
      // Parse the configuration
        if (($config = json_decode($wellKnown, true)) === null) {
            throw new OidcConfigurationResolveException(
                sprintf(
                    'Could not parse OIDC configuration. Response data: "%s"',
                    $wellKnown
                )
            );
        }
